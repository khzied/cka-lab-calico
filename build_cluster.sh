...
vagrant ssh master -c "echo 'Environment=\"KUBELET_EXTRA_ARGS=--node-ip=172.16.33.20\"' | sudo tee --append  /etc/systemd/system/kubelet.service.d/10-kubeadm.conf"
...
vagrant ssh node1 -c "echo 'Environment=\"KUBELET_EXTRA_ARGS=--node-ip=172.16.33.21\"' | sudo tee --append  /etc/systemd/system/kubelet.service.d/10-kubeadm.conf"
...
vagrant ssh node2 -c "echo 'Environment=\"KUBELET_EXTRA_ARGS=--node-ip=172.16.33.22\"' | sudo tee --append  /etc/systemd/system/kubelet.service.d/10-kubeadm.conf"
